...
public class CollMap<K, V> implements Map<K, V> {
    ...
    public V put(K key, V value) {
        ...
        int index = bucket(key);
        Entry<K, V> last = data[index];
        data[index] = new Entry(last, key, value, -1);
        return value;
    }//put


    public V putInt(K key, V value, int i) {
        if (key == null) throw new NullPointerException("not NULL argument required");
        int index = bucket(key);
        Entry<K, V> last = data[index];
        data[index] = new Entry(last, key, value, i);
        return value;
    }//put
    ...
    public static class Entry<K, V> implements Map.Entry<K, V> {
        ...
        public Entry(Entry<K, V> next, K key, V value, int ind) {
            this.next = next;
            this.key = key;
            this.value = value;
            this.index = ind;
